...
class MainController extends Controller
{
	public function index(Request $request){
		...
		if(Auth::check()){
			$data['todos'] = DB::table('todolist')->where('todolist.userid', Auth::user()->id)->leftjoin('status', 'todolist.status', '=', 'status.id')->select('todolist.*', 'status.typename')->orderBy('id', 'desc')->get();
		}
	...
	public function signup(Request $request){
		...
		if($request['email']){
			...
			$saveUser = $user->save();
			if($saveUser){
				$data['success'] = "Sign up successfull";
			}
			else{
				$data['error'] = "Failed to sign up please try again later, or contact administrator";
			}
